...
def init_uniform_rhok(rho_k,robots,w,h,win): # rho_k is a list of robot prefered distance
    rho_k.sort() #sort rho_k from highest to lowest
    distance_center = list()
    sorted_robots = list()
    for x in range(0,len(robots)):
        distance_center.append((x+1,distance_magnitude(distance_vector(robots[x].getCenter(),Point(w/2,h/2),win)[0],distance_vector(robots[x].getCenter(),Point(w/2,h/2),win)[1])))
    sorted(distance_center,key=lambda x:x[1])# sort n according to distance from center(0,0)
    for x in range(0,len(robots)):
        sorted_robots.append(robots[sorted(distance_center,key=lambda x:x[1])[x][0]-1])
    
    return sorted_robots,rho_k
...
def update_pairwisedistance(robot_j,rho_j,robot_k,rho_k,times,mu,win,robots):
    ...
    rj,yrj=get_nearbybots(robot_j,robots,win,xrj,yrj,theta,(45*np.pi/180))
